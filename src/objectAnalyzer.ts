...
export function analyze(typeObj: BaseType, lang: string, langObj: unknown, defaultLangObj: unknown): void {
  if (langObj === defaultLangObj) return;

  analyzeRecursively(typeObj, lang, langObj, '', defaultLangObj);
}

function analyzeRecursively(
  typeObj: BaseType,
  lang: string,
  langObj: unknown,
  varName: string,
  ...
  if (typeObj instanceof FunctionType) {
    ...
    for (const key of excessKeys) {
      Reflect.deleteProperty(langObj, key);
    ...
    for (const key of sharedKeys) {
      ...
      analyzeRecursively(memberTypeObj, lang, langObj[key], memberVarName, defaultLangObj[key]);
    }
  } else {
